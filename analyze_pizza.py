...
def parse_args() -> Namespace:
    ...
    arg_parser.add_argument('-f', '--data-file', required=True, help='.csv file or Excel spreadsheet that contains '
                                                                     'pizza club ratings.', type=str)
    arg_parser.add_argument('-o', '--output-file', required=False, help='filepath to output analysis to', type=str,
                            default='')
...
def is_individual_ratings(data_file_name: str) -> bool:
    ...
    return search("[Ii]ndividual", data_file_name) is not None
...
def get_individual_analysis(data: pd.DataFrame) -> pd.DataFrame:
    ...
    new_cols: typing.Dict[str, typing.Callable] = {'person_mean': pd.DataFrame.mean, 'person_range': get_range,
                                                   'person_min': pd.DataFrame.min, 'person_max': pd.DataFrame.max}
    new_rows: typing.Dict[str, typing.Callable] = {'category_mean': pd.DataFrame.mean, 'category_range': get_range,
                                                   'category_min': pd.DataFrame.min, 'category_max': pd.DataFrame.max}
